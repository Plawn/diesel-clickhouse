...
// =============================================================================
// HTTP Backend Integration Tests (requires running ClickHouse)
// =============================================================================
...
mod clickhouse_integration {
    ...
    fn test_create_and_drop_table() {
        ...
        let response = client
            ...
            .unwrap();
        assert!(response.status().is_success());
        let count: i64 = response.text().unwrap().trim().parse().unwrap();
        assert_eq!(count, 2);
    ...
    fn test_clickhouse_types() {
        ...
        let test_cases = vec![
            ...
            ("SELECT toInt64(-1234567890)", "-1234567890"),
            ("SELECT toFloat64(3.14159)", "3.14159"),
            ("SELECT 'Hello, World!'", "Hello, World!"),
            ("SELECT [1, 2, 3]", "[1,2,3]"),
            ("SELECT toDate('2024-01-15')", "2024-01-15"),
        ];
        ...
        for (query, expected) in test_cases {
            ...
            assert!(response.status().is_success(), "Query failed: {}", query);
    ...
    #[test]
    fn test_clickhouse_final_modifier() {
        if !clickhouse_available() {
            eprintln!("ClickHouse not available, skipping integration test");
            return;
        }
        ...
        let response = client
            ...
            .send()
            .unwrap();
        let count_no_final: i64 = response.text().unwrap().trim().parse().unwrap();

        // Query with FINAL (should return deduplicated)
        ...
        let response = client
            ...
            .send()
            .unwrap();
        let count_final: i64 = response.text().unwrap().trim().parse().unwrap();

        // FINAL should have <= rows than without FINAL
        assert!(count_final <= count_no_final);
        assert_eq!(count_final, 1); // Only one unique id
